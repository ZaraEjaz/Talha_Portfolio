...
// 1. We extract the paths here so we can reuse them in both SVGs
const SVGPaths = ({ pathLengths }: { pathLengths: MotionValue[] }) => (
  <>
    <motion.path
...
export const GoogleGeminiEffect = ({
  ...
  return (
    <div
      className={cn(
        "sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-black",
        className
      )}
    >
      <div className="relative z-20 text-center px-4 -mt-20 md:-mt-0">
      <p className="text-4xl md:text-7xl font-bold pb-2 text-center">
          {title || `FULL STACK PRECISION`}
        </p>
        <p className="text-base md:text-xl font-normal text-center text-neutral-400 max-w-lg mx-auto">
          {description ||
            `Crafting production-grade applications with robust performance and data-driven logic.`}
        </p>
      </div>
      ...
      */}
      <svg
        ...
        className="hidden md:block absolute bottom-0 w-full h-auto z-10 opacity-80 left-0"
      ...
      {/* --- SVG 2: MOBILE ONLY --- 
          - Visible on Mobile (block)
          - Hidden on Desktop (md:hidden)
          - ViewBox: 0 0 1440 1300 (Requested Value)
      */}
      <svg
        viewBox="0 0 1440 1300"
        xmlns="http://www.w3.org/2000/svg"
        className="block md:hidden absolute bottom-0 w-full h-auto z-10 opacity-80"
        preserveAspectRatio="xMidYMax slice"
      >
        <SVGPaths pathLengths={pathLengths} />
      </svg>
